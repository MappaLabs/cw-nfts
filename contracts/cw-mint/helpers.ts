...
interface CW721Instance {
  ...
  executeReceive: (senderAddress: string, amount: number) => Promise<string>
}
...
export const CW721 = (client: SigningCosmWasmClient, fees: Options['fees']): CW721Contract => {
  const use = (contractAddress: string): CW721Instance => {
    ...
    const executeReceive = async (senderAddress: string, amount: number): Promise<string> => {
      const result = await client.execute(senderAddress, contractAddress, { execute_receive: { amount } }, fees.exec)
      return result.transactionHash;
    }
